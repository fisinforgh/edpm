...
#This script UNINSTALL EDPMSoftEdu software
...
messageUNIS=$(echo "Welcome to the EDPM SoftEdu UNINSTALL wizard"
	      echo "You are about to uninstall  EDPM SoftEdu software"
	      echo "If you are completely sure, please clik on UNINSTALL"
	      )

echo "$messageUNIS" | (zenity --text-info --title="EDPM SoftEdu uninstall" --width=800 --height=400 --ok-label='UNINSTALL' --cancel-label='Exit')
if [ $? -eq 1 ]; then
    echo "Exit"
    exit 2
fi

(zenity --info --text="Please click CONTINUE to select \"Desktop\" Folder to UNISTALL EDPM SoftEdu desktop's icon" --title="EDPM SoftEdu UNINSTALL desktop icon" --ok-label='CONTINUE' --width=800 --height=400 )

dirDESK=$(zenity --file-selection   --directory --title="EDPM SoftEdu UNINSTALL choose a Desktop Folder" --width=800 --height=400)

manUNIS=$( echo "Remove EDPM SoftEdu software manually:"
	   ...
	   echo "# EDPMSoftEdu_v1 globally available executable "
	   echo "export PATH=\"/home/fisinfor/test_v1/EDPMSoftEdu_v1:\$PATH")
...
hiddenFILE=".pathDIR_EDPMSoftEdu_v1.txt"
dirNAME="EDPMSoftEdu_v1"
