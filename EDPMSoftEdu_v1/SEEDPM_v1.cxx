...
MyMainFrame::MyMainFrame(const TGWindow *p,UInt_t w,UInt_t h){
  ...
  //cout << pathDIR_ICONS << endl;
   ...
   TGLabel *fLabel1658 = new TGLabel(fGroupFrame1657,"Tiem interval [ s ] :",TGLabel::GetDefaultGC()(),TGLabel::GetDefaultFontStruct(),kChildFrame,ucolor);
   ...
   fLabelXT->MoveResize(16,16,100,14);
...
void MyMainFrame::DoHelp() {
  ...
  pave->AddText("AC raduis: Average cross section radius");
  ...
  pave->AddText("cross secction is assume to be, in");
  pave->AddText("average(a), equivalent to the circle's");
  pave->AddText("cross secctin(cs), computed by cicle's");
...
void MyMainFrame::DoDraw() {
  ...
  for( size_t i=0 ; i<nstep-1 ; ++i ){
    ...
    sprintf(cValVar, "x(t) = %0.4f [ m ]" , (float)y[i+1]);
    ...
    TString title_XY("Draged motion;x [m];y [m]");
